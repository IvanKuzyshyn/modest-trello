...
export const ManageColumnModal = ({ onCreate, onEdit, onClose, id }: Props) => {
    ...
    const column = useSelector<Column | undefined>((state) => getColumnById(state, id))
    useEffect(() => {
        if (!column) {
            return
        }
    ...
    const isNew = !column
    const handleChangeLabel = (event: React.ChangeEvent<HTMLInputElement>) => setLabel(event.target.value)
